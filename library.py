...
def mains():
    ...
    def add_student():
        ...
        if ret == 0:
            ...
            student_name.get(), int(roll_no.get()), branch.get(), int(phone.get()), book.get(), int(day.get()), int(month.get()), int(year.get()),int(rday.get()), int(rmonth.get()), int(ryear.get())))
            conn.commit()
            conn.close()
    ...
    if __name__== "__main__":
        ...
        label7 = Label(root, text="Reutrn Date:",font=("bold",11))
